package com.mapr.lab.sparkstreaming.utils
...
object TwitterHelper {
  ...
  def configureTwitterCredentials() {
    ...
    /**val file = new File(getClass.getResource("/twitter.txt").toURI)
    if (!file.exists) {
    ...
    val lines = Source.fromInputStream(getClass.getResourceAsStream("/twitter.txt")).getLines.filter(_.trim.size > 0).toSeq
